...
#print(connections.index((3,7,A1,E1,2e5)))
def simulate(nodes,free_nodes,truss,force,playback_speed,camera_object):
    ...
    position_dictionary_real = node_position_list(screenx, screeny, scale, real_displaced_position,camera_object,player.zoom) #convert world -> screen)
    ...
    while running:
        for event in pg.event.get():
            ...
            if event.type == pg.KEYDOWN:
                if event.key == pg.K_SPACE:
                    ...
                    u_global, reaction, axial_list, stress, UTS = solve_truss(nodes, free_nodes, truss, force*0)
                    real_displaced_position, displacement_ratio = compute_scale(nodes, u_global)
                    position_dictionary_real = node_position_list(screenx, screeny, scale, real_displaced_position,camera_object,player.zoom)
                    timer = 0
                    force_scale = 0
            camera_object.handle_clicking(event)
            camera_object.update_zoom(event,screenx,screeny,scale)
            ...
            if timer > fixed_dt:
                ...
                while True:
                    try:
                        ...
                        collapsed = True
                        break
                    #print(stress[7],UTS[7],stress[7]< UTS[7])
        ...
        if not collapsed : 
            ...
            position_dictionary_real = node_position_list(screenx, screeny, scale, prev_real_displaced_position,camera_object,player.zoom)
